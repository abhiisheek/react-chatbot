...
function getExampleFiles(examplesPath, componentName) {
  var exampleFiles = [];
  var exampleFilesExceptCSS = [];
  try {
    exampleFiles = getFiles(path.join(examplesPath, componentName));
    exampleFilesExceptCSS = exampleFiles.filter(filename => !filename.includes('.css') && !filename.includes('.png') && !filename.includes('.svg') && filename);
  } catch (error) {
    console.log(chalk.red(`No examples found for ${componentName}.`));
  }
  return exampleFilesExceptCSS;
}

function getDirectories(filepath) {
  return fs.readdirSync(filepath).filter(function(file) {
    return file !== 'utils' && file !== 'hocs' && fs
        .statSync(path.join(filepath, file))
        .isDirectory();
  });
}
